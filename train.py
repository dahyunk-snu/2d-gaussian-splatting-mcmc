...
def training(
    ...
    gaussians = GaussianModel(dataset.sh_degree, abs_grad)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if mcmc:
            total_loss = (
                total_loss + opt.opacity_reg * torch.abs(gaussians.get_opacity).mean()
            )
            total_loss = (
                total_loss + opt.scale_reg * torch.abs(gaussians.get_scaling).mean()
            )

            # or if you want to regularize by area:
            # scaling = gaussians.get_scaling
            # area = (scaling[..., 0] * scaling[..., 1]).abs()
            # total_loss = total_loss + opt.scale_reg * area.mean()
        ...
        with torch.no_grad():
            ...
            if (
                iteration > opt.densify_from_iter
                and iteration < opt.prune_until_iter
                and opt.use_prune_weight
                and abs_grad
            ):
                if (
                    iteration % img_num / img_num_modifier == 0
                    and iteration % opt.opacity_reset_interval
                    > img_num / img_num_modifier
                ):
